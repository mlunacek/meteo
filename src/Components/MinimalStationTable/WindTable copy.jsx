...
function windGustValue(s) {
    const d = getLatest(s);
    if (!d) return 0;
    return d.wind_gust > d.wind_speed ? Math.round(d.wind_gust) : 0;
}
                    ...
                    {data.map((s) => {
                        ...
                        return (
                            <TableRow key={s.metadata.id}>
                                ...
                                {/* --- NEW: literal "g" column with minimal padding --- */}
                                <TableCell
                                    align="center"
                                    aria-label="gust indicator"
                                    sx={{
                                        p: "0 !important",   // beats any table-wide td padding
                                        width: { xs: 16, sm: 18 },
                                        px: 0,       // tiny horizontal padding
                                        py: 0,         // minimal vertical padding
                                        lineHeight: 1, // keeps it compact
                                        color: "white",
                                        opacity: showG ? 0.7 : 0, // hide when no gust
                                    }}
                                >
                                    g
                                </TableCell>
                                ...
                                <TableCell
                                    sx={{
                                        paddingLeft: "0.5 !important",   // beats any table-wide td padding
                                        width: { xs: 72, sm: 80 },
                                        fontSize: { xs: 20, sm: 22 },
                                        fontWeight: 700,
                                        color: gustColor,
                                ...
                                >
                                    <Box sx={{ display: "inline-flex", alignItems: "baseline", gap: 0.5 }}>
                                        ...
                                        {showG && (
                                            <>
                                                <span style={{ opacity: 0.7, lineHeight: 1 }}>g</span>
