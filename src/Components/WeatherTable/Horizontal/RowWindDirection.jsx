...
export default function RowWindDirection({
    column,
    data,
    legendWidth,
    padding,
    ...
    return (
        <tr style={{ padding: `${padding}px`, height: `${cellHeight}px` }}>
            <StickyCell padding={padding} legendWidth={legendWidth} name="Dir" />
            {data.map((d, index) => (
                <td
                    style={{
                        width: `${cellWidth}px`,
                    ...
                    align="center">
                    <div style={generateStyle(d?.surface[column])}>{d?.surface[column] ? 4 : ""}</div>
