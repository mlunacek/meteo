...
export default function RowSounding({
    columnIndex,
    data,
    legendWidth,
    padding,
    ...
    const windSpeed = data.map((d) => d?.sounding?.wind_speed[columnIndex])
    const height = data.map((d) => d?.sounding?.height[columnIndex])
    ...
    return (
        <tr style={{ padding: `${padding}px`, height: `${cellHeight}px` }}>
            <StickyCell padding={padding} legendWidth={legendWidth} name={`${height[0].toFixed(1)}K`} units="ft." />
